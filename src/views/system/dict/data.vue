...
export default {
  ...
  created() {
    // 查询表格数据
    dictTypeApi.selectAllDictType().then(res => {
  ...
  methods: {
    ...
    getDictDataList() {
      this.loading = true
      dictDataApi.listForPage(this.addDateRange(this.queryParams, this.dateRange)).then(res => {
    ...
    handleUpdate(row) {
      ...
      this.open = true
      this.reset()
      // 根据dictCode查询一个字典数据信息
      dictDataApi.getById(dictCode).then(res => {
        this.form = res.data
      })
    },
    // 执行删除
    handleDelete(row) {
      ...
      this.$confirm('此操作将永久删除该字典数据，是否继续？', '提示', {
        ...
        this.loading = true
        dictDataApi.deleteByIds(dictCodes).then(() => {
          this.loading = false
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.dictCode === undefined) {
            dictDataApi.add(this.form).then(res => {
            ...
            // 做修改
            dictDataApi.update(this.form).then(res => {
