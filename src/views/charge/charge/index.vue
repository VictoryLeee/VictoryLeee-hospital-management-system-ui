  ...
  <div v-loading="loading" :element-loading-text="loadingText">
    ...
    <el-card v-if="careHistory.registrationId !== undefined" style="margin-top:5px">
      ...
      <div style="margin-bottom: 5px">
        ...
        <el-button type="danger" size="small" icon="el-icon-bank-card" @click="handlePayWithCash">现金支付</el-button>
        <el-button type="danger" size="small" icon="el-icon-bank-card" @click="handlePayWithZFB">支付宝支付</el-button>
    ...
    <el-card v-if="careOrderList.length>0" style="margin-top:5px">
      <el-collapse v-model="activeNames">
        <el-collapse-item v-for="(item, index) in careOrderList" :key="index" :name="index">
          <template slot="title">
            <span style="font-weight: 550;">{{ index+1 }}、<span v-if="item.coType === '0'" style="color:red">药品处方</span><span v-else style="color:green">检查处方</span> 、金额:<span style="color:red">￥ {{ item.allAmount }}</span></span>
          </template>
          <el-table
            ref="refTable"
            v-loading="loading"
...
<script>
import { getNoChargeRegistrationAndCareHistoryAndCareOrderDataByRegId, addOrderChargeWithCash, addOrderChargeWithZFB, getOrderChargeByOrderId } from '@/api/charge/ordercharge'
...
export default {
  ...
  methods: {
    ...
    handleQuery() {
      ...
      getNoChargeRegistrationAndCareHistoryAndCareOrderDataByRegId(this.registrationId).then(res => {
    ...
    handlePayWithCash() {
      ...
      this.$confirm('是否确定创建订单并使用现金支付?', '提示', {
        ...
        addOrderChargeWithCash(orderChargeFromDto).then(() => {
    ...
    handlePayWithZFB() {
      ...
      this.$confirm('是否确定创建订单并使用支付宝支付?', '提示', {
        ...
        addOrderChargeWithZFB(orderChargeFromDto).then(res => {
    ...
    createOrderCharge() {
      ...
      this.itemObjList.filter(item => {
        ...
        orderChargeFromDto.orderChargeItemList.push(i)
      })
      return orderChargeFromDto
    },
    // 清空病历信息和处方列表
    resetCurrentParams() {
      this.careHistory = {}
